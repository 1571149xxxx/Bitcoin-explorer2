...
public class TransferServiceImpl implements TransferService {
    ...
    @Override
    //通过Hash导入块
    public void setBlockByHash(String blockHash, boolean isNull) throws Throwable {
        if(isNull==true){
       blockmapper.truncate();
        ...
        while (whileHash != null && !"".equals(whileHash + "")) {
            ...
            for (int i = 0; i < tx.size(); i++) {
                setTx(tx.getJSONObject(i),whileHash,date);
            }
    ...
    public void setTx(JSONObject tx, String blockhash, Date time) throws Throwable {
